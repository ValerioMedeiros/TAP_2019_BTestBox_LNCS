...
\begin{document}
%
\title{BTestBox - A tool for testing B translators and coverage of B models \thanks{Supported by NPAD to provide the supercomputer.}}
%
%\titlerunning{Abbreviated paper title}
% If the paper title is too long for the running head, you can set
% an abbreviated paper title here
%
\author{Diego de Azevedo Oliveira \inst{1}\orcidID{0000-1111-2222-3333} \and
Val{\'e}rio Medeiros Jr\inst{2,3}\orcidID{1111-2222-3333-4444} \and
David D{\'e}harbe\inst{3}\orcidID{2222--3333-4444-5555} \and
Martin A. Musicante \inst{3}\orcidID{2222--3333-4444-5555}
}
%
...
Translators are used to perform the conversion from B representation to another computer language such as C, ADA, and LLVM. Each translator can translate a subset of B, called B0. Moreover, a translator can have a particularity and only translate a subset of B0. %Such restrictions are not well documented and may confuse the user. 
...
%During this section, we present how BTestBox uses the Hoare's Logic to create a predicate, our tool relays on the Hoare Triple. With the Hoare Triple it is possible to calculate how a commands can change the state. 
...
%In order to generate the test cases, BTestBox writes the predicate related to each previous generated path of the model under test or until the chosen criteria is achieved. Even more, different criterion has separate goals, for example, with the PC is necessary to test every path and for the operation ``RussianMultiplication'' is necessary to test the three. Although, for ST if the path 1 is feasible, then just it need to be executed. Choosing the BC for the operation in the Figure \ref{fig:russianMultGraph}, the minimum necessary paths to be executed is two.
...
%After this procedure, BTestBox found the necessary test cases for achieving BC for the operation ``RussianMultiplication''. And they are shown in the table \ref{tab:TestCases}.
...
%We are currently developing an approach using threads to decrease the time for running one implementation with more than one operation. But, the experiments are not concluded due the project still in progress.
...
The work is supported by the Federal Institute of the Rio Grande do Norte, the Federal University of Rio Grande do Norte and CAPES (Coordination of Improvement of Higher Level Personnel). This work was grammatically reviewed by Antonio Henrique Nepomuceno Coelho (Federal English Teacher at IFRN).
