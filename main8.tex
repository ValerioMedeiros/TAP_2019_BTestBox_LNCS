...
certifications issued by regulation entities demand or recommend testing
the generated software using a code coverage criteria.
We propose improvements for the  \textit{BTestBox}, a tool for automatic generation
...
The tool uses a constraint solver to produce tests, thus being able to
...
numbers that are one order of magnitude better than the ones obtained
in the tool's previous version.

%\keywords{First keyword  \and Second keyword \and Another keyword.}
\keywords{B Method, Model-based testing, Code coverage.}
\end{abstract}
%
%
%
...
The B method of software development~\cite{abrial2005b} produces the source code by the successive refinement from an abstract model. 
...
Our tool receives as inputs a B implementation, the target translator, one compiler, one coverage criterion, the folder project, and the logic expression solver (ProB ~\cite{1_leuschel_2017}) %\footnote{ProB is an animator, constraint solver and model checker for B models~\cite{1_leuschel_2017}.}
directory. 
...
In this Section, we describe how \textit{BTestBox} can manipulate the user files, insert ```get'' and ``set'' operations, and execute the test with the previously calculated inputs. 
...
% Martin estamos mantendo em ingles britanico, ela parece que se confundiu nisso.
% Estamos conversando aqui https://hangouts.google.com/call/N-lPJvbfE-2qny8gpehfAEEE   (MARITN ) VEJA SE TIVER TEMPO
